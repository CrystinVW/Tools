...
def makeFile():
    file = "http://quote.yahoo.com/d/quotes.csv?s=EURCAD=X+CADEUR=X+EURUSD=X+USDEUR=X+EURJPY=X+JPYEUR=X+USDCAD=X+CADUSD=X+USDJPY=X+JPYUSD=X+CADJPY=X+JPYCAD=X+&f=sl1&e=.csv"
...
def main():
    ...
    if bf.hasNegativeCycle():
        ...
        while result:
            ...
            print("{0} to {1} @ {2} = {3:.2f} {4}".format(edge.fromVertex(), edge.toVertex(), fxData[key], balance,
                                                    edge.toVertex()))
    else:
